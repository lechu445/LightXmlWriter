...
namespace Tools.Text.Xml.Tests
{
  public class Utf8XmlWriterTest
  {
    ...
    {
      public static readonly XmlExcapedName Person = XmlExcapedName.Encode("Person");
    }
    ...
    //{
    //  var ms = new MemoryStream();
    //  using (var subject = new LightStreamXmlWriter(ms))
    //  {
    //    subject.WriteStartElement(XmlNames.Person);
    ...
    //  using (var subject = new LightStreamXmlWriter(ms))
    //  {
    //    OTA_Standard_XML_Writer_LightStreamXmlWriter.Write(subject);
    //  }
    //  var actual = ms.ToUtf8String();
